...
export function CheckPasswordStrength({ password }: Props) {
  ...
  const atLeastOneUppercaseLetter = (password: string) => /[A-Z]/.test(password);
  const atLeastOneLowercaseLetter = (password: string) => /[a-z]/.test(password);
  const atLeastOneNumber = (password: string) => /\d/.test(password);
  function testingPasswordStrength(password?: string) {
    if (!password) return 'WEAK'; let points = 0;
  ...
  function generateColors(strength: string): string[] {
    const NEUTRAL = '#E0E0E0'; const WEAK = '#FF6B6B';
    const MEDIUM = '#FFD166'; const STRONG = '#06D6A0';
    switch (strength) {
      case 'WEAK':
  ...
  return (
    <>
      <Box marginTop={2} width={'60%'}>
        <Box display="flex" gap="5px" marginBottom={1}>
          {colors.map((color, index) => (
            <Box key={index} flex={1} height="5px" bgcolor={color} borderRadius="2px" />
          ))}
        </Box>
        ...
        {passwordStrength === 'WEAK' && (
          <Typography variant="body2" color="text.secondary">It must have at least 10 characters, 1 uppercase, 1 lowercase, and 1 digit.</Typography>
        )}
      </Box>
    </>
