...
export default function Logo({ sx = {} }: LogoProps) {
  ...
  return (
    <Stack sx={{ ...style.root, ...sx } as SxProps} component={Link} href='/'>
      {
        organization.logo ?
          <Image src={organization.logo} alt='logo' className='client-logo' height={50} width={250} priority />
          :
          <Stack sx={style.logo}>
            <Stack sx={style.iconBox}>E</Stack>
            <Typography sx={style.name}>EMS.</Typography>
          </Stack>
      }
    </Stack>
  )
